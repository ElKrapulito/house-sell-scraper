...
class HouseDatabase:
    ...
    def get_house_by_full_address(self, fullAddress: str) -> Dict:
        house_list = self.search_houses(fullAddress=fullAddress)
        if len(house_list) <= 0:
            return {}
        else:
            return house_list[0]
...
def main():
    ...
    for house in all_houses:
        print(
            f"ID: {house['id']}, Address: {house['houseNumber']} {house['address']}, {house['city']}, {house['state']} {house['zipCode']}"
        )
...
if __name__ == "__main__":
    db = HouseDatabase("redfin.db")
    house = db.get_house_by_address("2826 Jefferson Dr, Alexandria, VA 22303")

    house2 = db.get_house_by_full_address(
        "418 N Saint Asaph St Unit 1/2, Alexandria, VA 22314"
    )
