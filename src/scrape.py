...
def scrape_redfin_page(response):
        try:
            ...
            existing_properties = list(map(map_housedb_to_scrape_house, existing_properties))
...
def scrape_redfin_search_page(url: str, page_count:int=1, total_page: int=1):
    ...
    try:
        ...
        if page_count == 1:
            ...
            page_home_count = page.find('div', class_='descriptionSummary').find('div', class_='homes').get_text(strip=True)
            ...
            if page_home_count > 41: 
                ...
                pages_count = pages_count[len(pages_count) - 1].find('span', class_='ButtonLabel').get_text(strip=True)
...
def filter_properties(properties: list):
    return filter(
        lambda property : 
            property[csv_headers.TAX_PRICE.value] * cg.RATE >= property[csv_headers.PRICE.value] if property[csv_headers.TAX_PRICE.value] and property[csv_headers.PRICE.value] else False,
        properties)
